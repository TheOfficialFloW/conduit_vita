...
static NameToMethodID name_to_method_ids[] = {
  ...
  { "FileGetExpansionName", FILE_GET_EXPANSION_NAME },
...
float GetGamepadAxis(int axis) {
  ...
  switch (axis) {
    ...
    case 5: // R2
    {
      if (axis == 4 && pad.buttons & SCE_CTRL_L1) {
        val = 1.0f;
        break;
      } else if (axis == 5 && pad.buttons & SCE_CTRL_R1) {
        val = 1.0f;
        break;
      }
    }
...
char *FileGetExpansionName(int unk) {
  return "main.obb";
}
...
void *CallObjectMethodV(void *env, void *obj, int methodID, uintptr_t *args) {
  switch (methodID) {
    case FILE_GET_EXPANSION_NAME:
      return FileGetExpansionName(args[0]);
...
void jni_load(void) {
  ...
  *(uintptr_t *)(fake_vm + 0x18) = (uintptr_t)GetEnv;
  ...
  int (* JNI_OnLoad)(void *vm, void *reserved) = (void *)so_symbol(&conduit_mod, "JNI_OnLoad");
